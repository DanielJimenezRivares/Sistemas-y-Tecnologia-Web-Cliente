...
// GET /valoraciones/reviews/recientes?limit=10
// Devuelve las últimas valoraciones (de todos los espacios), ordenadas por timestamp desc
router.get("/reviews/recientes", (req, res) => {
  try {
    const db = readDB();
    ...
    // Flatten: [{ espacio_cultural_id, rating, review?, username, timestamp }]
    const all = [];
    for (const [espacioId, rec] of Object.entries(db)) {
      if (!rec || !Array.isArray(rec.reviews)) continue;

      for (const r of rec.reviews) {
        all.push({
          review_id: r.id,
          espacio_cultural_id: String(espacioId),
          rating: r.rating,
    ...
    if (total === 0) {
      return res.json({ total: 0, limit, count: 0, data: [] });
    }

    // Sort: más recientes primero (fechas inválidas al final)
    all.sort((a, b) => {
    ...
    return res.json({
      total,        // nº total real de reviews en el sistema
      limit,        // limit aplicado (normalizado)
      count: data.length, // nº real devuelto
      data,
    });
  } catch (err) {
    console.error("GET /valoraciones/recientes error:", err);
    return res.status(500).json({ error: "internal server error" });
  }
});
...
router.get("/reviews/:id?", (req, res) => {
  try {
    ...
    // Ensure each review has username (fallback to "Anónimo")
    const page = sorted.slice(s, e + 1).map(r => ({
      review_id: r.id,
      rating: r.rating,
      review: r.review ?? null,
      username: (typeof r.username === "string" && r.username.trim()) ? r.username.trim() : "Anónimo",
      timestamp: r.timestamp
    }));
